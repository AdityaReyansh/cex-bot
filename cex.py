            ...
            for category in game_config['upgradeCardsConfig']:
                for upgrade in category['upgrades']:
                    ...
                    if current_level < len(upgrade['levels']):
                        ...
                        cost, ccy, effect, effect_ccy, unk = upgrade['levels'][current_level]
            ...
            for i, account_data in enumerate(data):
                try:
                    ...
                    if result['status'] == 'ok':
                        ...
                        print(f"========== Account {i + 1} | {Fore.GREEN}{user_data['first_name']}{Style.RESET_ALL} ==========")
                        ...
                        try:
                            divisor = pow(10, int(user_data['precision_BTC']))
                            balance_btc = float(user_data['balance_BTC'])
                            balance_usd = float(user_data['balance_USD'])
                            balance_cexp = float(user_data['balance_CEXP'])
                            multi_taps_energy = int(user_data['multiTapsEnergy'])
                            multi_taps_energy_limit = int(user_data['multiTapsEnergyLimit'])

                            self.log(f"Balance BTC: {balance_btc / divisor}", 'success')
                            self.log(f"Balance USD: {balance_usd}", 'success')
                            self.log(f"Balance CEXP: {balance_cexp}", 'success')
                            self.log(f"Energy: {multi_taps_energy} / {multi_taps_energy_limit}", 'info')
                        except Exception as e:
                            self.log(f"Error processing balances: {str(e)}", 'error')

                        if do_swap:
                            if balance_btc > 0.0 and swap_percentage > 0:
                                try:
                                    btc_to_convert = round((balance_btc / divisor) * (swap_percentage / 100), int(user_data['precision_BTC']))
                        ...
                        if buy_cards_decision:
                            self.process_upgrades(account_data, session, balance_usd)
